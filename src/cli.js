...
const FigmaIconsController = require('./controllers/main')
...
// function getFigmaIconInFile(document){
//   return new Promise((resolve, reject) => {
//     spinner.start('Find icons in all pages')

//     let icons = []
//     let pages = document.children;
//     icons = pages.filter(p => !p.name.includes('_'))
//                  .map(p => p.children.map(f => { 
  ...
  return new Promise((resolve) => {
    ...
    figmaClient.get(`/files/${config.fileId}`)
      .then((res) => {
        ...
        resolve({ document: res.data.document })
